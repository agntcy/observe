        ...
        try:
            ...
            while True:
                try:
                    ...
                    response = await self.send_message_and_wait_response(session, message)
        ...
        try:
            for iteration in range(iterations):
                ...
                for i, message in enumerate(messages):
                    try:
                        response = await self.send_message_and_wait_response(session, message)
                        print(f"[{iteration+1}/{iterations}][{i+1}/{len(messages)}] Sent: {message}")
                        print(f"[{iteration+1}/{iterations}][{i+1}/{len(messages)}] Received: {response}")
                ...
                if iteration < iterations - 1:
                    print(f"Completed iteration {iteration + 1}, waiting before next...")
...
async def main():
    ...
    parser.add_argument(
        "--enable-mls",
        action="store_true",
        help="Enable MLS (Message Layer Security)"
    )
    parser.add_argument(
        "--message",
        type=str,
        help="Single message to send (batch mode)"
    )
    parser.add_argument(
        "--messages",
        nargs='+',
        help="Multiple messages to send (batch mode)"
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=1,
        help="Number of iterations for batch mode"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
