...
def create_jwt_identity(
    ...
    for trust_domain, encoded_jwks in spire_bundle.items():
        ...
        try:
            ...
            if "keys" in jwks_json:
                ...
                logger.info(f"  Added {len(jwks_json['keys'])} keys from {trust_domain}")
            else:
    ...
    logger.info(f"Combined JWKS contains {len(all_keys)} total keys from {len(spire_bundle)} trust domains")
...
async def initialize_tracing(enable_opentelemetry: bool = False, log_level: str = "info"):
    """
...
async def create_slim_app(
    ...
    if jwt_path and spire_bundle_path and audience:
        ...
        provider, verifier = create_shared_secret_identity(local_identity, shared_secret)
    else:
        raise ValueError(
...
class SlimConfig:
    ...
    def to_connection_config(self) -> Dict[str, Any]:
        ...
        return {
            "endpoint": f"http://{self.endpoint}" if not self.endpoint.startswith("http") else self.endpoint,
